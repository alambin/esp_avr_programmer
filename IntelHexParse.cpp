...
void
IntelHexParse::ParseLine(byte* hexline)
{
...
byte*
IntelHexParse::GetLoadAddress()
{
    return _loadAddress;
}

void
IntelHexParse::GetLoadAddress(byte* hexline)
{
    char buff[3];
    buff[2] = '\0';
...
byte*
IntelHexParse::GetData(byte* hexline, int len)
{
    int  start = 9;
    int  end   = (len * 2) + start;
    char buff[3];
...
int
IntelHexParse::GetAddress(byte* hexline)
{
    char buff[5];
    buff[0] = hexline[3];
    buff[1] = hexline[4];
...
int
IntelHexParse::GetLength(byte* hexline)
{
    char buff[3];
    buff[0] = hexline[1];
    buff[1] = hexline[2];
...
int
IntelHexParse::GetRecordType(byte* hexline)
{
    char buff[3];
    buff[0] = hexline[7];
    buff[1] = hexline[8];
